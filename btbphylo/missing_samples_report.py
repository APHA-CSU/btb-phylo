...
"""
    ...
    excluded from ViewBovine.
"""


def get_excluded(df_wgs_deduped, df_wgs_included):
    """
        Returns a DataFrame for WGS samples that are filtered. The
        DataFrame is in the form of df_summary (see Readme).
    """
...
def exclusion_reason(df_wgs_excluded, df_clade_info, outliers):
    ...
    for clade, row in df_clade_info.iterrows():
        ...
        df_report.loc[mask, "Ncount"] = df_wgs_excluded.\
            loc[df_wgs_excluded["group"] ==
                clade].apply(lambda sample: "Pass" if sample["Ncount"] <=
                             row["maxN"] else "Fail", axis=1)
    # add outlier column
    df_report["outlier"] = \
...
def missing_data(df_report, missing_wgs_samples, missing_cattle_samples,
    ...
    """
        Appends columns to the report DataFrame, detailing whether each
        sample is present in WGS, cattle and movement datasets.
    """
    ...
    df_report_missing_data["cattle_data"] = \
        df_report_missing_data["Submission"].map(lambda x: x not in
                                                 missing_cattle_samples)
    df_report_missing_data["movement_data"] = \
        df_report_missing_data["Submission"].map(lambda x: x not in
                                                 missing_movement_samples)
...
def add_eartag_column(df_report, df_cattle, df_movement):
    ...
    df_report_eartag["eartag"] = \
        df_report.apply(lambda x: df_cattle.
                        loc[df_cattle["CVLRef"] == x["Submission"],
...
def report(df_wgs_deduped, df_wgs_included, cattle_movements_path,
    ...
    """
        ...
        Parameters:
            ...
            duplicates removed. In the form of df_summary

            df_wgs_included (pandas DataFrame object): WGS samples
            included in ViewBovine. In the form of df_summary
