...
class TestDeDuplicate(unittest.TestCase):
    def test_remove_duplicates(self):
        ...
        # test with categorical kwarg 
        test_df = pd.DataFrame({"Submission":pd.Series(["1", "2", "2", "2", "1", "3", "1"], dtype="object"),
                                "pcMapped":pd.Series([0.1, 0.2, 0.3, 0.1, 0.1, 0.1], dtype=float),
                                "bar":pd.Series(["Pass", "Pass", "Fail", "Fail", "Fail", "Pass", "Fail"], dtype="object")})
        nptesting.assert_array_equal(de_duplicate.remove_duplicates(test_df, pcMapped="max", bar="Pass").values,
                                     pd.DataFrame({"Submission":["2", "3", "1"], 
                                                   "pcMapped":[0.4, 0.1, 0.6], "Ncount":[4, 6, 1], "foo":[40, 60, 70]}).values)
    ...
    def test_get_indexes_to_remove(self):
        # test max
        test_df = pd.DataFrame({"Submission":pd.Series(["1", "2", "2", "2", "1", "3"], dtype="object"),
                                "Outcome":pd.Series(["Pass", "Pass", "Pass", "Pass", "Pass", "Pass"], dtype="object"),
        ...
        # test categorical
        test_df = pd.DataFrame({"Submission":pd.Series(["1", "2", "2", "2", "1", "3"], dtype="object"),
                                "Outcome":pd.Series(["Pass", "Pass", "Fail", "Pass", "Fail", "Pass"], dtype="object"),
                                "pcMapped":pd.Series([0.1, 0.2, 0.3, 0.4, 0.1, 0.6], dtype=float),
                                "Ncount":pd.Series([1, 2, 3, 4, 5, 6], dtype=float)})
        pd.testing.assert_index_equal(de_duplicate.get_indexes_to_remove(test_df, "Outcome", "Pass"),
                                      pd.Index([2, 4]), check_order=False)